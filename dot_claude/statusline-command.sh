...
# 0. Research session ticker (keyed by session_id)
ticker=""
...
# 2. Model in bold white (distinct from dim separators)
model_part=""
if [[ -n "$model" ]]; then
    model_part="${BOLD_WHITE}${model}${RESET}"
fi

# 3. Cost in yellow, formatted as $X.XX
cost_part=""
if [[ -n "$cost" ]]; then
